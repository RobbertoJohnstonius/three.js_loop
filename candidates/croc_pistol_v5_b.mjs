...
export function createAsset() {
    ...
    let orangeGuardGeo = new THREE.BufferGeometry();
    const vertices = [];
    const colors = [];
    const indices = [];
    const width = 0.46, height = 0.1, depth = 0.44;
    const divisionsX = 3, divisionsY = 1, divisionsZ = 1;
    const uvs = [];

    for (let x = 0; x <= divisionsX; x++) {
        for (let y = 0; y <= divisionsY; y++) {
            for (let z = 0; z <= divisionsZ; z++) {
                const tx = x / divisionsX, ty = y / divisionsY, tz = z / divisionsZ;
                vertices.push(width * (2 * tx - 1), -height * (2 * ty - 1), depth * (2 * tz - 1));
                const c = hash(tx - 0.5, ty - 0.5, tz - 0.5);
                const [r, g, b] = [0.82 + c * 0.06, 0.41 + c * 0.08, 0.12 + c * 0.04];
                colors.push(r, g, b);
                uvs.push(tx, ty);
            }
        }
    ...
    const n = (divisionsX + 1) * (divisionsY + 1) * (divisionsZ + 1);
    for (let i = 0; i < n; i++) {
        const x = i % (divisionsX + 1), y = Math.floor(i / (divisionsX + 1)) % (divisionsY + 1), z = Math.floor(i / ((divisionsX + 1) * (divisionsY + 1)));
        if (x < divisionsX && y < divisionsY && z < divisionsZ) {
            indices.push(
                i, i + divisionsX + 1, i + (divisionsX + 1) * (divisionsY + 1),
                i, i + (divisionsX + 1) * (divisionsY + 1), i + (divisionsX + 1) * (divisionsY + 1) + 1
            );
        }
    }

    orangeGuardGeo.setAttribute('position', new THREE.Float32BufferAttribute(vertices, 3));
    orangeGuardGeo.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3));
    orangeGuardGeo.setAttribute('uv', new THREE.Float32BufferAttribute(uvs, 2));
    orangeGuardGeo.setIndex(indices);
