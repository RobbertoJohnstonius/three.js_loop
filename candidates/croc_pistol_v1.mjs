...
export function createAsset() {
    ...
    // ── Body / slide — 6×4×4 subdivisions → 256 tris ─────────────────────────
    let bodyGeo = new THREE.BoxGeometry(1.56, 0.42, 0.44, 6, 4, 4);
    ...
    // ── Gold ornamental rail — 10×2×2 subdivisions → 176 tris ────────────────
    let railGeo = new THREE.BoxGeometry(1.58, 0.12, 0.46, 10, 2, 2);
    ...
    // ── Gold spines on top of rail — 5 × ConeGeometry(4 sides) ──────────────
    const spineMat = new THREE.MeshStandardMaterial({
        color: 0xffffff, vertexColors: true, roughness: 0.10, metalness: 0.92
    });
    [-0.55, -0.28, 0.0, 0.28, 0.55].forEach(sx => {
        let sg = new THREE.ConeGeometry(0.058, 0.10, 4, 1);
        posNoise(sg, 0.010);
        sg = applyFaceColors(sg, t => [0.82 + t * 0.08, 0.66 + t * 0.08, 0.18 + t * 0.05]);
        const sm = new THREE.Mesh(sg, spineMat);
        sm.name = 'spike';
        sm.position.set(sx, 0.665, 0);
        group.add(sm);
    });

    // ── Barrel — SHORT FAT stub, rotated to +X axis ───────────────────────────
    // CylinderGeometry(r, r, h, radSeg, hSeg) → rotateZ → aligns along X
    let barrelGeo = new THREE.CylinderGeometry(0.14, 0.14, 0.22, 12, 2);
    ...
    let muzzleGeo = new THREE.CylinderGeometry(0.17, 0.14, 0.06, 10, 1);
    ...
    // ── Grip — 4×10×4 subdivisions → 384 tris, GREEN dominant scale ──────────
    let gripGeo = new THREE.BoxGeometry(0.50, 0.64, 0.43, 4, 10, 4);
    ...
    // Made taller (0.20) and wider (0.50) to be clearly visible from camera angle
    let guardGeo = new THREE.BoxGeometry(0.50, 0.20, 0.44, 4, 2, 2);
    ...
    // Position further down so it clears the grip and is unambiguously visible
    guardMesh.position.set(-0.04, -0.08, 0);
    group.add(guardMesh);
