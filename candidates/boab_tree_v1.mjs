...
export function createAsset() {
  ...
  function applyFaceColors(geo, palette) {
    ...
    for (let f = 0; f < nFaces; f++) {
      ...
      const t = hashPos(cx, cy, cz) * 0.7 + hashPos(cx*2.1, cy*2.1, cz*2.1) * 0.3;
      const r = t < 0.5
  ...
  // CylinderGeometry(topR, botR, height, radSeg, heightSeg)
  ...
  for (let i = 0; i < tpos.count; i++) {
    ...
    // y in [-0.65, 0.65]; map to [0,1]
    const yN = (y + 0.65) / 1.3;
    // Boab bulge: Gaussian peak at yN ≈ 0.35
  ...
  for (const { pos, r } of clusters) {
    ...
    let s = Math.round((pos[0]+pos[1]+pos[2]) * 9999) | 1;
    for (let i = 0; i < lp.count; i++) {
      s = (s * 1664525 + 1013904223) & 0xffffffff;
      const disp = ((s >>> 0) / 0xffffffff) * 0.08 - 0.04;
